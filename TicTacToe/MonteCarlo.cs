...
namespace MonteCarlo
{
    ...
    public class MonteCarloTreeSearch
    {
        private class Node<TPlayer, TAction> : IMctsNode<TAction> where TPlayer : IPlayer where TAction : IAction
        {
            ...
            public void BuildTree(int maxIterations, long timeBudget)
            {
                Console.Write("Finding computer's move...");
                ProgressBar.ProgressBar progress = new();
                var iterations = 0;
                var timer = Stopwatch.StartNew();
                while (iterations++ < maxIterations && timer.ElapsedMilliseconds < timeBudget)
                {
                    var node = this;
                    ...
                    progress.Report((double)iterations / maxIterations > (double)timer.ElapsedMilliseconds / timeBudget ? 
                        (double)iterations / maxIterations : (double)timer.ElapsedMilliseconds / timeBudget);
                }
        ...
        public static IEnumerable<IMctsNode<TAction>> GetTopActions<TPlayer, TAction>(IState<TPlayer, TAction> state, int maxIterations, long timeBudget) where TPlayer : IPlayer where TAction : IAction
        {
            ...
            root.BuildTree(maxIterations, timeBudget);
